...
for counter in {1..50}; do
	...
	-local_times\
	-num_stimts 8 \
	-stim_times 1 ${prefix}'_all_neg_cueonset_iter_'${counter}'.txt' 'BLOCK(4,1)' \
	-stim_label 1 All_neg_cue \
	-stim_times 2 ${prefix}'_mixed_expel_pos_cueonset_iter_'${counter}'.txt' 'BLOCK(4,1)' \
	-stim_label 2 Mix_Ex_P_cue \
	-stim_times 3 ${prefix}'_all_pos_cueonset_iter_'${counter}'.txt' 'BLOCK(4,1)' \
	-stim_label 3 All_pos_cue \
	-stim_times 4 ${prefix}'_mixed_expel_neg_cueonset_iter_'${counter}'.txt' 'BLOCK(4,1)' \
	-stim_label 4 Mix_Ex_N_cue \
	-stim_times 5 ${prefix}'_all_neg_encodeonset_iter_'${counter}'.txt' 'BLOCK(6,1)' \
	-stim_label 5 All_neg_enc \
	-stim_times 6 ${prefix}'_mixed_expel_pos_encodeonset_iter_'${counter}'.txt' 'BLOCK(6,1)' \
	-stim_label 6 Mix_Ex_P_enc \
	-stim_times 7 ${prefix}'_all_pos_encodeonset_iter_'${counter}'.txt' 'BLOCK(6,1)' \
	-stim_label 7 All_pos_enc \
	-stim_times 8 ${prefix}'_mixed_expel_neg_encodeonset_iter_'${counter}'.txt' 'BLOCK(6,1)' \
	...
	-x1D 3dD_encVcue_BLOCK_${prefix}_${counter}.xmat.1D > 3dD_encVcue_BLOCK_${prefix}_${counter}.txt
	echo ${prefix}'_'${counter} $( tail -1 3dD_${prefix}_${counter}.txt ) >> ITIOptim_3dD_BLOCK_encVcue${prefix}.txt
